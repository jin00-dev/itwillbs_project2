...
public class UserController {
	...
	public String joinPOST(UserVO vo,Model model) throws Exception{
		...
		// 전달정보 저장(회원가입 정보)
		logger.debug("암호화 전 vo "+vo);
		String inputPass= ""; //입력한 비밀번호
		String pass="";  //입력한 비밀번호를 암호화
		
		inputPass = vo.getUser_pw();
		pass = passEncoder.encode(inputPass); 
		vo.setUser_pw(pass);
		
		logger.debug("inputPass "+inputPass);
	...
	// 로그인 POST (받은 거 가져가서 DB에서 확인하는 것)
		...
		public String loginPOST(UserVO vo,
			...
			if (resultVO != null) { // 일치하는 아이디 존재시
				session.setAttribute("vo", resultVO.getUser_id());
...
//				model.addAttribute("vo", resultVO);
	...
	// 회원정보 수정POST
		...
		@RequestMapping(value = "/child",method = RequestMethod.POST)
			...
			//http://localhost:8088/user/pwUpdateView
			...
			public String pwUpdateView() throws Exception{
				logger.debug("비번 변경 페이지 진입"); 
				return "/user/pwUpdateView";	
			}	
			...
			public int pwCheck(UserVO vo) throws Exception{
				String user_pw = uService.pwCheck(vo.getUser_id());
				                          //평문               , 암호화된 거                            	
				if( passEncoder.matches(vo.getUser_pw(), user_pw)) { //비번이 없거나 다를때
					return 1;
				}
				return 0;
			}
			...
			public String pwUpdate(UserVO vo,
				...
				logger.debug("****"+user_pw2);
				uService.pwUpdate(user_id, hashedPw);
				session.invalidate();
	...
	public String deletePOST(@ModelAttribute UserVO vo, HttpSession session) {
