...
<div class="container abx">
	<div class="card mb-3">
		<div class="card-body">
			<div class="pt-4 pb-2">
						<h3 class="card-title text-center">회원 약관</h3>
				<form action="/user/join" method="get">
					<div class="form-group">
							<textarea rows="10" class="form-control" readonly="readonly">
회원 약관에는 회원이 꼭 알아두어야 할 회사와 회원의 관계에 대한 중요한 내용이 포함되어 있습니다.
가입을 원하시면 아래의 회원 약관을 반드시 읽고 동의 버튼을 눌러주세요.

제1장 총칙
제1조 (목적)
이 약관은 전기통신사업법령 및 정보통신망이용촉진 및 정보보호 등에 관한 법률에 따라 세모클래스(이하 `회사`)에서 제공하는 모든 서비스(이하 `서비스`)의 이용절차,조건등 서비스 이용과 관련한 회사와 회원의 권리 및 의무에 관련된 사항을 규정함을 목적으로 합니다.
...
세모클래스는 고객의 회원정보를 소중하게 관리합니다.

세모클래스의 개인정보보호와 관련된 정책을 알고 싶은 분은 [개인정보 취급방침]을 클릭하세요.
	</textarea>
					</div>
					<div class="form-group d-grid gap-2 mt-3">
						<!-- label 안에 input tag를 넣으면 박스 뿐만 아니라 글자를 클릭해도 동작한다. -->
						<label><input type="checkbox" required="required"
							title="약관에 동의하셔야 회원가입이 가능합니다."> 약관에 동의합니다</label>
						<button class="btn btn-primary">가입하러가기</button>
					</div>
				</form>
			</div>
		</div>
