			...
			<h3>${user_name }님의마이페이지입니다.</h3>
				...
				<table class="table">
					...
					<c:forEach var="i" items="${list }" varStatus="status">
						<tr>
							...
							<td>${i.paid_amount }</td>
							<td><button class="btn1" onclick="location.href='/class/paymentInfo?user_id=${user_id}&order_num=${i.order_num }'">상세내역</button></td>
				...
				<c:if test="${user_type eq 1}">
					<div class="align-right">
						<button class="btn btn-primary" data-bs-toggle="tab" onclick="location.href='/board/uploadForm';" aria-selected="false" role="tab" tabindex="-1">사업자 클래스 등록</button>
					</div>
				</c:if>
				<nav aria-label="Page navigation">
					<ul class="pagination justify-content-center">
						<c:if test="${pageVO.preview }">
							<li class="page-item"><a class="page-link" href="/class/paymentList?page=${pageVO.startPage - 1 }" aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
							</a></li>
						</c:if>

						<c:forEach var="i" begin="${pageVO.startPage }" end="${pageVO.endPage }" step="1">
							<li ${pageVO.cri.page == i? 'class="active"':'' } class="page-item"><a class="page-link" href="/class/paymentList?page=${i }">${i }</a></li>
						</c:forEach>

						<c:if test="${pageVO.next }">
							<li class="page-item"><a class="page-link" aria-label="Next" href="/class/paymentList?page=${pageVO.endPage + 1 }"> <span aria-hidden="true">&laquo;</span>
							</a></li>
						</c:if>
					</ul>
				</nav>
